...
var WORD_LIST_RIKAIDO = ["immersion", "ajatt", "anki", "refold"];
...
var nextWord = function(room){
    ...
    if(room.wordList.length <= 3){
        room.wordList = JSON.parse(JSON.stringify(room.selectedWordList === "Default" ? WORD_LIST : WORD_LIST_POE));
		if(room.customWordList != ""){
			var newWordList = room.customWordList.split(",");
			if(newWordList.length >= 4){
				room.wordList = newWordList;
